...
class CifarOODDataset(Dataset):

    def __init__(self, ds_name, transform) -> None:
        super().__init__()
        data_path = f'{OOD_PATH}/{ds_name}/'
        self.ds = datasets.ImageFolder(data_path, transform=transform)
        self.labels = [-1] * len(self.ds)
...
def get_cifar10(settype,ds_name):
    if settype == 'train':
...
def get_cifar100(settype,ds_name):
    if settype == 'train':
        trans = gen_cifar_transform()
...
def get_tinyimagenet(settype,ds_name):
    if settype == 'train':
...
def get_imagenet1000(settype,ds_name):
    if settype == 'train':
...
def get_ood_inaturalist(settype,ds_name):
    if settype == 'train':
        raise Exception("OOD iNaturalist cannot be used as train set.")
    else:
        return LargeOODDataset(ds_name,gen_imagenet_transform(crop = True, testmode=True))

def get_cifar_ood(settype,ds_name):
    if settype == 'train':
        raise Exception("OOD imagenet_crop cannot be used as train set.")
    else:
        return CifarOODDataset(ds_name,gen_cifar_transform(crop = True, testmode=True))
...
ds_dict = {
    ...
    'imagenet_crop' : get_cifar_ood,
    'imagenet_resize' : get_cifar_ood,
    'lsun_crop' : get_cifar_ood,
    'lsun_resize' : get_cifar_ood,
}
...
def get_ds_with_name(settype,ds_name):
    ...
    if key not in cache_base_ds.keys():
        cache_base_ds[key] = ds_dict[ds_name](settype, ds_name)
...
def get_combined_dataloaders(args,settings):
    ...
    test_cifar100 = get_combined_dataset('test', settings['test_cifar100'])
    test_svhn = get_combined_dataset('test', settings['test_svhn'])
    return torch.utils.data.DataLoader(train_labeled, batch_size=args.bs, shuffle=istrain_mode, num_workers=workers,pin_memory=True,drop_last = use_droplast) if train_labeled is not None else None,\
            torch.utils.data.DataLoader(test_cifar100, batch_size=args.bs, shuffle=False, num_workers=test_workers,pin_memory=args.gpu != 'cpu') if test_cifar100 is not None else None,\
            torch.utils.data.DataLoader(test_svhn, batch_size=args.bs, shuffle=False, num_workers=test_workers,pin_memory=args.gpu != 'cpu') if test_svhn is not None else None
...
def load_partitioned_dataset(args,ds):
    ...
    return a,b,c,ds_classnum_dict[settings['name']]
